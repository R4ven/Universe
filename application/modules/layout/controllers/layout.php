...
class Layout extends MX_Controller {
    ...
    private $layout = 'layout/page/standard';
    private $page_title;
    private $body_class;
    private $js_files;
    private $css_files;
    ...
    function __construct () {
        ...
        $this->js_files = array(
			'public/js/lib/jquery.min.js' => 'public/js/lib/jquery.min.js',
        ...
        $this->css_files = array(
			...
			'public/css/bootstrap-responsive.min.css' => 'public/css/bootstrap-responsive.min.css',
     ...
     */
    public function addJs ($file, $remove = false) {
        if ($remove) {
            if (is_array($file)) {
                foreach ($file as $k=>$v) {
    ...
    private function load_js () {
        ...
        if (count($this->js_files)) {
            foreach ($this->js_files as $file) {
                ...
                //$file = 'application/modules/'.$file[0].'/views/skin/'.implode('/',array_slice($file, 1));
     ...
     */
    public function addCss ($file, $remove = false) {
        if ($remove) {
            if (is_array($file)) {
                foreach ($file as $k=>$v) {
	...
	public function addBlock ($path, $block_data = '') {
        return $this->load->view( $this->template_path($path), $block_data, true);
    }
	...
	public function setBlock($key, $path, $block_data = '') {
        $this->block[$key][$path] = $this->block($path, $block_data);
        return $this;
    }
    ...
    public function load ($module = null, $filename = null) {
        ...
        $layout = array(
            // TODO: use $this->config->item('modules_locations') for modules path
            simplexml_load_file(APPPATH.'modules/layout/views/layout/default.xml'),
            simplexml_load_file(APPPATH.'modules/'.$module.'/views/layout/default.xml')
        );*/
        /*
        if ($filename === null) {
            $filename = $this->router->fetch_class();
        }
        $layout[] = simplexml_load_file(APPPATH.'modules/'.$module.'/views/layout/'.$filename.'.xml');
