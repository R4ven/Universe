...
class Template_Controller extends CI_Controller
{
    ...
    public function _load_view()
    {
        if ($this->view !== false)
        {
            ...
            $data['content'] = $this->prerendered_data;
            $data['content'] .= $this->load->view($view, $this->data, true);
			
			$data['topmenu'] = $this->load->view('frontend/menu/topbar', array(), true);
			$data['sidemenu'] = $this->load->view('frontend/menu/sidebar', array(), true);
			$data['footer'] = $this->load->view('frontend/menu/footer', array(), true);
			...
			if(!empty($this->CSSFiles)) {
				foreach($this->CSSFiles AS $key => $val) {
					$data['CSSFiles'] .= '<link rel="stylesheet" href="' . base_url() . 'public/css/'.$val.'" />';
				}
			}
            ...
            if (!isset($this->layout)) {
                if (defined('CLIENT_PATH') && file_exists(CLIENT_PATH . 'views/layouts/' . $this->router->class . '.php')) { 
                    ...
                    $this->load->view('frontend/layouts/frontendLayout.php', $data);
                }
            } else  
